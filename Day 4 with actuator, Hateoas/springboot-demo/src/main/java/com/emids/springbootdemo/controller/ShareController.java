...
public class ShareController {
	...
	public ResponseEntity<Object> getShareByName(@PathVariable String shareName) throws ShareNotFoundException {		
		...
		CompanyShare share = shareOpt.get();		
		Link link1 = WebMvcLinkBuilder.linkTo(ShareController.class).withRel("allShares");
		share.add(link1);		
		Link link2 = WebMvcLinkBuilder.linkTo(ShareReviewController.class).slash(shareName).withRel("review");
		share.add(link2);		
		ResponseEntity<Object> responseEntity = new ResponseEntity<>(shareOpt.get(),HttpStatus.OK);	
	...
	public CompanyShare getShareByUUID(@PathVariable UUID shareId) throws ShareNotFoundException {	
		...
		CompanyShare share =  shareOpt.get();
		Link link = WebMvcLinkBuilder.linkTo(ShareController.class)
				.slash("/name").slash(share.getShareName()).withRel("getByName");
		share.add(link);
		Link link3 = WebMvcLinkBuilder.linkTo(ShareController.class).withRel("post");
		Link link2 = WebMvcLinkBuilder.linkTo(ShareController.class).withRel("allShares");
		share.add(link2);
		share.add(link3);
		return share;
	}
